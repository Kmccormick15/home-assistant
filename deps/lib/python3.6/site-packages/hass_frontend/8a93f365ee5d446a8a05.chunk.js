...
`}static get properties(){return{hass:{type:Object},entries:{type:Array,value:[]}}}_formatTime(date){return Object(format_time.a)(new Date(date),this.language)}_formatDate(date){return Object(format_date.a)(new Date(date),this.language)}_needHeader(change,index){if(!index)return!0;const current=this.get("when",change.base[index]),previous=this.get("when",change.base[index-1]);return current&&previous&&current.substr(0,10)!==previous.substr(0,10)}_computeIcon(domain){return Object(domain_icon.a)(domain)}entityClicked(ev){ev.preventDefault();this.fire("hass-more-info",{entityId:ev.model.item.entity_id})}}customElements.define("ha-logbook",ha_logbook_HaLogbook);var localize_mixin=__webpack_require__(9);class ha_panel_logbook_HaPanelLogbook extends Object(localize_mixin.a)(polymer_element.a){static get template(){return html_tag.a`
